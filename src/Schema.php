...
final class Schema
{
    ...
    public static function ensureUpgrades($pdo = null)
    {
        ...
        $driver = $pdo->getAttribute(PDO::ATTR_DRIVER_NAME) === 'sqlite' ? 'sqlite' : 'mysql';
        foreach (self::marcasStatements($driver) as $sql) {
            $pdo->exec($sql);
        }
    ...
    /**
     * Tablas de marcas (instalación nueva y bases ya existentes).
     *
     * @param string $driver
     * @return list<string>
     */
    private static function marcasStatements($driver)
    {
        if ($driver === 'sqlite') {
            return array(
                "CREATE TABLE IF NOT EXISTS crm_marcas (
                    ...
                    incluir_global INTEGER NOT NULL DEFAULT 1,
